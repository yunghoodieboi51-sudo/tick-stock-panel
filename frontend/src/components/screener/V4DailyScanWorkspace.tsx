...
export function V4DailyScanWorkspace({ onBack }: { onBack: () => void }) {
  ...
  const completed = useMemo(() => runs.data?.items.filter(item => item.state === 'COMPLETED') ?? [], [runs.data])
  const selectedIndex = completed.findIndex(item => item.run_id === runId)
  const previousId = selectedIndex >= 0 ? completed[selectedIndex + 1]?.run_id : undefined
  const previous = useQuery({ queryKey: QK.dailyScanRun(previousId ?? ''), queryFn: () => api.dailyScanRun(previousId ?? ''), enabled: Boolean(previousId) })

  useEffect(() => {
    if (runId || !runs.data) return
    const latest = completed[0] ?? runs.data.items[0]
    if (latest) setRunId(latest.run_id)
  }, [completed, runId, runs.data])
  ...
  const comparison = useMemo(() => {
    if (!run.data?.result || !previous.data?.result) return null
    const current = new Set(run.data.result.candidates.map(item => item.symbol))
    const prior = new Set(previous.data.result.candidates.map(item => item.symbol))
    return {
      added: [...current].filter(symbol => !prior.has(symbol)).length,
      retained: [...current].filter(symbol => prior.has(symbol)).length,
      removed: [...prior].filter(symbol => !current.has(symbol)).length,
    }
  }, [previous.data, run.data])
  const state = run.data?.manifest.state
  const dataDate = run.data?.result?.trade_date
  const historical = Boolean(dataDate && dataStatus.data?.enriched?.latest_date && dataDate !== dataStatus.data.enriched.latest_date)
...
function Row({ item, rejected }: { item: DailyScanCandidate; rejected: boolean }) {
  ...
  const href = `/trading-plan?run_id=${encodeURIComponent(item.run_id)}&symbol=${encodeURIComponent(item.symbol)}`
  const fundamental = (!item.fundamental.evidence || item.fundamental.evidence.availability === 'MISSING') && item.fundamental.status === 'PASS' ? 'UNKNOWN' : item.fundamental.status
  return <tr className="border-b border-border/60">{!rejected && <td className="p-2 font-mono">{item.rank ?? '—'}</td>}<td className="p-2 font-mono">{item.symbol}</td><td className="p-2">{item.name ?? '—'}</td><td className="p-2 font-mono">{rejected ? '—' : <>{price(item.execution_reference_price)}{missingRaw && <span className="ml-1 text-warning">RAW unavailable</span>}</>}</td><td className="p-2 font-mono">{price(item.final_score)}</td><td className="p-2">{item.entry_pattern_primary ?? '—'}</td><td className="p-2">{fundamental}</td>{rejected && <td className="p-2 text-warning">{item.fundamental.reason_codes.join(', ') || '—'}</td>}<td className="p-2"><Link to={href} className="text-accent hover:underline">详情 / Plan</Link></td></tr>
}
